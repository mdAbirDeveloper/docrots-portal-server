...
async function run() {
    try {
        const appointmentOptionsCollention = client.db('doctorPortal').collection('appointmentOptions');
        ...
        app.get('/appointmentOption', async (req, res) => {
            ...
            const query = {};
            const options = await appointmentOptionsCollention.find(query).toArray();
            ...
            options.forEach(option => {
                ...
                const remainingSlote = option.slots.filter(slot => !bookedSlots.includes(slot));
                option.slots = remainingSlote;
            })
            res.send(options)
        });

        //load anly one property on the data by project funtion
        app.get('/appoinementSpecialty', async (req, res) => {
            const query = {};
            const result = await appointmentOptionsCollention.find(query).project({ name: 1 }).toArray();
            res.send(result);
        });
        ...
        */
        ...
        //         }
        //     };
        //     const result = await appointmentOptionsCollention.updateMany(filter, updatedDoc, options);
        //     res.send(result);
        // })
...
app.listen(port, () => {
    console.log(`doctors porlat running on port ${port}`)
});
